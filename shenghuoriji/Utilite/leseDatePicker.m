...
@interface leseDatePicker ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation leseDatePicker

+(instancetype)shareManager{
    
    static leseDatePicker * manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [[leseDatePicker alloc] init];
        }
    });
    return manager;
}
...
-(void)createView{
    ...
    [trueButton setTitle:@"OK" forState:UIControlStateNormal];
...
-(void)showWithType:(PickerType)pickerType title:(NSString * _Nullable)title time:( NSString * _Nullable )timeStr backTime:(void (^)(NSString * backTimeStr))backTime{
    ...
    if (timeArr.count == count) {//需要显示传入的时间
        ...
        for (int i = 0; i<count; i++) {
            [_pickerView selectRow:[_dataArray[i] containsObject:timeArr[i]]?[_dataArray[i] indexOfObject:timeArr[i]]:0 inComponent:i animated:YES];
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (_selDateArray.count == _dataArray.count) {
        ...
        if (row == [pickerView selectedRowInComponent:component]) {
            NSMutableArray * array = [[NSMutableArray alloc]initWithArray:@[@"Y",@"M",@"D",@"时",@"分"]];
