...
- (BOOL)deleteDataWith:(NSString *)dbName Identifier:(NSString *)identifier IdentifierValue:(NSString *)identifierValue {
    ...
    // 处理不同表命名(name、name.sqlite)获取有效表名
    NSString *dbPath = [[self class] getValidDBName:dbName];
    NSString *deletesql = [NSString stringWithFormat:@"DELETE FROM t_%@ WHERE %@ = ?",dbPath,checkObjectNotNull(identifier)?identifier:PRIMARYKEY];
    BOOL isDelete = [_fmdb executeUpdate:deletesql,identifierValue];
...
- (BOOL)modifyDataWith:(NSString *)dbName Key:(NSString *)key Value:(id )value Key2:(NSString *)key2 Value2:(id )value2  value3:(id)value3{
    ...
    // 处理不同表命名(name、name.sqlite)获取有效表名
    NSString *dbPath = [[self class] getValidDBName:dbName];
    NSString *sql = [NSString stringWithFormat:@"UPDATE t_%@ SET %@ = ?,%@ = ? WHERE id = %@",dbPath,key,key2,value3];
    
    BOOL isModifySuccess = [_fmdb executeUpdate:sql withArgumentsInArray:@[value,value2]];
    if(!isModifySuccess) NSLog(@"modify error:%@",_fmdb.lastErrorMessage);
...
- (NSMutableArray *)_queryDataFormTable:(NSString *)dbName where:(NSString *)str {
    ...
    FMResultSet *rs=[_fmdb executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE state=%@", dbPath, str]];
    NSMutableArray *dataArray = [NSMutableArray array];
    while ([rs next]) {
        //查询的都是以name为a的字段
        NSLog(@"++++++++++ name: %@", [rs stringForColumn:@"name"]);
        NSData *dataQuery = [rs dataForColumn:@"status"];
        //解档将字节转换为字典
        NSDictionary * dictData = [NSKeyedUnarchiver unarchiveObjectWithData:dataQuery];
        NSLog(@"--------  %@", dictData);
        [dataArray addObject:dictData];
    }
    return dataArray;
}
- (NSArray *)queryStateDataWith:(NSString *)dbName {
    ...
    NSString *dbPath = [[self class] getValidDBName:dbName];
    NSLog(@"%@",_fmdb);
    NSString *querySql = [NSString stringWithFormat:@"SELECT * FROM t_%@ WHERE state='0'",dbPath];
...
- (NSArray *)queryStateDataWith:(NSString *)dbName expressTitle:(NSString *)title {
    ...
    NSString *dbPath = [[self class] getValidDBName:dbName];
    NSLog(@"%@",_fmdb);
    NSString *querySql = [NSString stringWithFormat:@"SELECT * FROM t_%@ WHERE express_title='%@'",dbPath,title];
...
- (NSArray *)queryStateDataWith:(NSString *)dbName expressTitle:(NSString *)title destination_name:(NSString *)name{
    ...
    NSString *dbPath = [[self class] getValidDBName:dbName];
    NSLog(@"%@",_fmdb);
    NSString *querySql = [NSString stringWithFormat:@"SELECT * FROM t_%@ WHERE express_title='%@' and destination_name='%@'",dbPath,title,name];
